...
class GameScene: SKScene {
    ...
    func launch(angle: Int, velocity: Int){
        ...
        let speed = Double(velocity) / 2
        
        let radias = deg2rad(degrees: angle)
        ...
        if currentPlayer == 1{
            ...
            let impluse = CGVector(dx: cos(radias) * speed, dy: sin(radias) * speed)
    ...
    func createPlayers(){
        ...
        player2.position = CGPoint(x: player2Buildings.position.x, y: player2Buildings.position.y + ((player2Buildings.size.height + player1.size.height) / 2))
        addChild(player2)
